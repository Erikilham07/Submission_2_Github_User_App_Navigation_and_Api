...
class DetailUserActivity : AppCompatActivity() {
    ...
    private fun setUserData(user: SearchDetail) {
        binding.txName.text = user.name
        binding.txUsername.text = user.username
        binding.txCompany.text = user.company
        binding.txLocation.text = user.location
        binding.txRepository.text = resources.getString(R.string.Repository, user.publicRepos)
        Glide.with(this)
            .load(user.avatar)
            .circleCrop()
